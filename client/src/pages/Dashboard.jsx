...
export default function Dashboard({ user, onLogout }) {
  ...
  // recebimentos do mês (parcelas a receber)
  const [monthRef, setMonthRef] = useState(() => String(todayIso()).slice(0, 7));
  const [confirmReceive, setConfirmReceive] = useState(null); // { saleId, number, client_name, due_date, value }
  ...
  const monthReceivable = useMemo(() => {
    ...
    for (const s of sales) {
      for (const it0 of (s.installments || [])) {
        ...
        if (!isInMonth(it.due_date, monthRef)) continue;
    ...
    items.sort((a, b) => String(a.due_date).localeCompare(String(b.due_date)) || (a.saleId - b.saleId) || (a.number - b.number));
    return { total, items };
  }, [sales, monthRef]);

  const requestMarkReceived = (row) => {
    setReceiveMsg('');
    setConfirmReceive(row);
  };

  const confirmMarkReceived = async () => {
    if (!confirmReceive) return;
    const { saleId, number } = confirmReceive;
    setReceiveMsg('');

    const sale = sales.find(s => s.id === saleId);
    if (!sale) {
      setReceiveMsg('❌ Venda não encontrada.');
      setConfirmReceive(null);
      return;
    }

    try {
      const payloadInstallments = (sale.installments || []).map(it0 => {
        const it = normalizeInstallmentForUi(it0);
        const isTarget = Number(it.number) === Number(number);

        const paid = isTarget || it.status === 'paid' || !!it.paid_date;
        ...
        return {
          ...
          paid_date: paid ? (isTarget ? todayIso() : (it.paid_date || todayIso())) : null
        };
      });

      const resp = await fetch(`/api/sales/${saleId}/installments`, {
        method: 'PUT',
      ...
      if (!resp.ok) {
        setReceiveMsg('❌ Não foi possível marcar como recebido. Faça login novamente e tente de novo.');
        return;
      }
      ...
      setReceiveMsg('❌ Erro ao marcar como recebido.');
    }
  };
  ...
  return (
    <Shell darkMode={darkMode}>
            ...
            {tab === 'overview' && (
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <Pill icon={BarChart3} label={`A receber (${monthRef})`} value={fmtBRL(monthReceivable.total)} tone="cyan" />
                </div>
                ...
                <div className="rounded-3xl border border-slate-200/60 dark:border-white/10 bg-white/70 dark:bg-white/5 backdrop-blur-xl p-6 shadow-xl">
                  <SectionTitle
                    ...
                    subtitle="Soma das parcelas que vencem no mês selecionado (somente as não recebidas)."
                  />

                  <div className="mt-4 flex flex-col sm:flex-row gap-3 sm:items-end sm:justify-between">
                    <div className="flex items-center gap-3">
                      <div className="text-sm font-semibold text-slate-700 dark:text-slate-200">Mês</div>
                      <input
                        type="month"
                        value={monthRef}
                        onChange={(e) => setMonthRef(e.target.value)}
                        className="px-3 py-2 rounded-2xl border border-slate-200/70 dark:border-white/10 bg-white/80 dark:bg-white/5 text-slate-800 dark:text-slate-100 outline-none focus:ring-2 focus:ring-violet-500/30"
                      />
                    </div>

                    <div className="text-sm text-slate-700 dark:text-slate-200">
                      <span className="text-slate-500 dark:text-slate-400">Total a receber:</span>{' '}
                      <span className="font-semibold">{fmtBRL(monthReceivable.total)}</span>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 rounded-2xl border border-slate-200/60 dark:border-white/10 overflow-hidden">
                    <div className="max-h-80 overflow-auto">
                      <table className="w-full text-sm">
                        <thead className="sticky top-0 bg-slate-50/95 dark:bg-slate-900/60 backdrop-blur border-b border-slate-200/60 dark:border-white/10">
                          <tr className="text-left">
                            <th className="px-4 py-3 font-semibold">Venc.</th>
                        ...
                        <tbody>
                          {monthReceivable.items.length === 0 ? (
                            <tr>
                            ...
                            monthReceivable.items.map((row) => (
                              <tr key={`${row.saleId}-${row.number}`} className="border-b border-slate-200/50 dark:border-white/10 hover:bg-slate-50/60 dark:hover:bg-white/5">
                                <td className="px-4 py-3 whitespace-nowrap">{fmtDate(row.due_date)}</td>
                                <td className="px-4 py-3">{row.client_name}</td>
                                <td className="px-4 py-3">{row.product}</td>
                                <td className="px-4 py-3 whitespace-nowrap">#{row.number}</td>
                                <td className="px-4 py-3 whitespace-nowrap">{fmtBRL(row.value)}</td>
                                <td className="px-4 py-3 whitespace-nowrap">{statusLabel(row.status)}</td>
                                <td className="px-4 py-3 whitespace-nowrap text-right">
                                  <button
                                    onClick={() => requestMarkReceived(row)}
                                    className="px-3 py-2 rounded-2xl bg-emerald-500/15 text-emerald-700 dark:text-emerald-200 border border-emerald-500/30 hover:bg-emerald-500/25 transition"
                                  >
                                    Marcar recebido
                                  </button>
                                </td>
      ...
      <Modal
        ...
        onClose={() => { setConfirmReceive(null); setReceiveMsg(''); }}
        widthClass="max-w-xl"
      >
        <div className="space-y-4">
          <div className="text-slate-700 dark:text-slate-200">
            Tem certeza que deseja marcar esta parcela como <span className="font-semibold">recebida</span>?
          </div>

          {confirmReceive && (
            <div className="rounded-2xl border border-slate-200/60 dark:border-white/10 bg-slate-50/70 dark:bg-white/5 p-4 text-sm text-slate-700 dark:text-slate-200">
              <div><span className="text-slate-500 dark:text-slate-400">Cliente:</span> {confirmReceive.client_name}</div>
              <div><span className="text-slate-500 dark:text-slate-400">Vencimento:</span> {fmtDate(confirmReceive.due_date)}</div>
              <div><span className="text-slate-500 dark:text-slate-400">Parcela:</span> #{confirmReceive.number}</div>
              <div><span className="text-slate-500 dark:text-slate-400">Valor:</span> {fmtBRL(confirmReceive.value)}</div>
            </div>
          )}

          {receiveMsg && (
            <div className="text-sm text-rose-500">{receiveMsg}</div>
          )}

          <div className="flex items-center justify-end gap-2">
            <button
              onClick={() => { setConfirmReceive(null); setReceiveMsg(''); }}
              className="px-4 py-2 rounded-2xl border border-slate-200/70 dark:border-white/10 hover:bg-slate-50 dark:hover:bg-white/5 transition text-slate-700 dark:text-slate-200"
            >
              Cancelar
            </button>
            <button
              onClick={confirmMarkReceived}
              className="px-4 py-2 rounded-2xl bg-emerald-500 text-white font-semibold shadow-lg shadow-emerald-500/25 hover:opacity-95 transition"
            >
              Sim, recebido
            </button>
          </div>
