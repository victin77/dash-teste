...
function SaleForm({ open, onClose, onSave, consultants, user, editing }) {
  ...
  const [product, setProduct] = useState(editing?.product || 'ImÃ³vel');
  ...
  useEffect(() => {
    ...
    setProduct(editing?.product || 'ImÃ³vel');
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setError('Confira os campos obrigatÃ³rios.');
    } finally {
      setSaving(false);
    }
  };
    ...
    <Modal open={open} onClose={onClose} title={editing ? 'Editar venda' : 'Nova venda'} widthClass="max-w-4xl">
      <form onSubmit={submit} className="space-y-5">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <TextInput label="NÂº do cliente" value={clientNumber} onChange={setClientNumber} placeholder="Ex: 12345" />
          <SelectInput
            label="Produto"
            required
            value={product}
            onChange={setProduct}
            options={['ImÃ³vel','Auto','Moto','Agro','ServiÃ§os'].map(x=>({ value:x, label:x }))}
          />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <TextInput label="% ComissÃ£o" required type="number" value={commissionPct} onChange={setCommissionPct} placeholder="Ex: 0.8" />
        ...
        <div className="rounded-3xl border border-slate-200/60 dark:border-white/10 bg-white/70 dark:bg-white/5 p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="text-xs text-slate-500 dark:text-slate-400">O total da venda Ã© a soma das cotas.</div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <TextInput label="CrÃ©dito gerado (R$)" type="number" value={creditGenerated} onChange={setCreditGenerated} />
...
function normalizeInstallmentForUi(it) {
  // status "overdue" automÃ¡tico no front se estiver pendente e vencido
...
export default function Dashboard({ user, onLogout }) {
  ...
  useEffect(() => {
    ...
    api.recebimentos(cashMonth, isAdmin ? cashConsultantId : null)
      ...
      .catch(() => { if (alive) setCashflowError('NÃ£o foi possÃ­vel carregar os recebimentos desse mÃªs.'); });
    return () => { alive = false; };
  }, [cashMonth, cashConsultantId, isAdmin]);

  useEffect(() => {
    // toda vez que abrir detalhes, prepara parcelas editÃ¡veis
    if (!details) {
  ...
  const ranking = useMemo(() => {
    // Ranking por VENDAS totais (nÃ£o exibe comissÃ£o).
    ...
    if (Array.isArray(rankingAll) && rankingAll.length) {
      return rankingAll
        .map(r => ({ name: r.name || 'â€”', totalSales: Number(r.totalSales || 0), salesCount: Number(r.salesCount || 0) }))
        .sort((a, b) => b.totalSales - a.totalSales);
    }
    // fallback (caso o endpoint falhe): calcula com base nas vendas carregadas
    const map = new Map();
    for (const s of sales) {
      const key = s.consultant_name || 'â€”';
          ...
          tooltip: {
            ...
            callbacks: {
              ...
              label: (ctx) => {
                ...
                return [`Total: ${fmtBRL(total)}`, `NÃ­vel: ${tier.label}`];
              }
            }
          }
        },
  ...
  const downloadExcel = async () => {
    ...
    if (!resp.ok) {
      alert('NÃ£o foi possÃ­vel exportar agora. FaÃ§a login novamente e tente de novo.');
      return;
    }
  ...
  const importExcel = async (file) => {
    ...
    const ok = confirm(
      'Importar este arquivo Excel (.xlsx) para o sistema?\n\nDica: use o arquivo exportado pelo prÃ³prio Dashboard.'
    );
    if (!ok) return;
  ...
  const setInstallmentUiStatus = (number, newUiStatus) => {
    setEditingInstallments(prev => prev.map(it => {
      ...
      if (newUiStatus === 'bill_overdue') {
        // boleto atrasado (cliente nÃ£o pagou) - mantÃ©m como pendente para regras automÃ¡ticas,
        // mas marca a flag para exibir corretamente.
        return { ...it, status: 'pending', bill_overdue: 1, paid_date: null };
      }
      // pending: se estÃ¡ vencida, vai aparecer como atrasada automaticamente (normalize)
  ...
  const saveInstallments = async () => {
    ...
    try {
      // ao salvar: se nÃ£o estÃ¡ pago e venceu, manda como overdue (pra ficar persistido)
      ...
      if (!resp.ok) {
        setInstallmentsMsg('Erro: NÃ£o foi possÃ­vel salvar as parcelas. FaÃ§a login novamente e tente de novo.');
        return;
      }
  ...
  const saveQuotas = async () => {
    ...
    try {
      ...
      setQuotasMsg('Erro: NÃ£o foi possÃ­vel salvar as cotas.');
    } finally {
        ...
        <div className="w-full px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="text-sm text-slate-600 dark:text-slate-400">
                  {isAdmin
                    ? 'Admin'
                    : `Consultor - ${consultants.find(c => c.id === user.consultant_id)?.name || 'â€”'}`}
                </div>
              </div>
            </div>
          ...
          <div className="pb-4">
            <div className="flex flex-wrap gap-2">
              <TabButton active={tab==='overview'} onClick={()=>setTab('overview')} icon={FileText} label="VisÃ£o geral" />
              ...
              {isAdmin && <TabButton active={tab==='settings'} onClick={()=>setTab('settings')} icon={Settings2} label="ConfiguraÃ§Ãµes" />}
            </div>
          </div>
        </div>
      </header>
          ...
          <>
            {tab === 'overview' && (
              <div className="space-y-8">
                {summary && (
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      ...
                      <Pill icon={BarChart3} label="ComissÃ£o hoje" value={fmtBRL(summary.today?.commission_total || 0)} tone="emerald" />
                      <Pill icon={FileText} label="Vendas (7 dias)" value={`${summary.last7?.sales_count || 0}`} tone="violet" />
                      <Pill icon={BarChart3} label="ComissÃ£o (mÃªs)" value={fmtBRL(summary.month?.commission_total || 0)} tone="violet" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  <Pill icon={BarChart3} label="ComissÃ£o total" value={fmtBRL(kpis.total)} tone="violet" />
                ...
                <div className="rounded-3xl border border-slate-200/60 dark:border-white/10 bg-white/70 dark:bg-white/5 backdrop-blur-xl p-6 shadow-xl">
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <SectionTitle icon={FileText} title="Recebimentos do mÃªs" subtitle="Parcelas que vencem no mÃªs selecionado" />
                    <div className="flex items-center gap-3">
                      {isAdmin && (
                        <>
                      ...
                      <div className="text-sm text-slate-500 dark:text-slate-400">MÃªs</div>
                      <input
                        type="month"
                        value={cashMonth}
                  ...
                  <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div className="rounded-2xl border border-slate-200 dark:border-white/10 p-4">
                      <div className="text-xs text-slate-500 dark:text-slate-400">Total no mÃªs</div>
                    ...
                    <div className="rounded-2xl border border-slate-200 dark:border-white/10 p-4">
                      <div className="text-xs text-slate-500 dark:text-slate-400">PerÃ­odo</div>
                      <div className="text-sm font-semibold text-slate-900 dark:text-white">
                        {cashflow?.range ? `${fmtDate(cashflow.range.start)} â†’ ${fmtDate(cashflow.range.end)}` : 'â€”'}
                      </div>
                      ...
                      <tbody>
                        {(cashflow?.installments || []).map((it) => {
                          ...
                          return (
                            <tr key={`${it.sale_id}-${it.installment_number}`} className="border-t border-slate-200/60 dark:border-white/10">
                              ...
                              <td className="py-2 pr-4 whitespace-nowrap">{it.installment_number}Âª</td>
                        ...
                        {(!cashflow?.installments || cashflow.installments.length === 0) && (
                          <tr className="border-t border-slate-200/60 dark:border-white/10">
                            <td colSpan={(isAdmin && !cashConsultantId) ? 7 : 6} className="py-4 text-center text-slate-500 dark:text-slate-400">Nenhuma parcela vence nesse mÃªs.</td>
                          </tr>
                        )}
                      </tbody>
                    </table>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div className="rounded-3xl border border-slate-200/60 dark:border-white/10 bg-white/70 dark:bg-white/5 backdrop-blur-xl p-6 shadow-xl">
                    <SectionTitle icon={BarChart3} title="Ranking (Jogo de Vendas)" subtitle="Todos veem o ranking â€¢ Mostra somente o total de vendas" />
                    <div className="mt-4">
                  ...
                  <div className="rounded-3xl border border-slate-200/60 dark:border-white/10 bg-white/70 dark:bg-white/5 backdrop-blur-xl p-6 shadow-xl">
                    <SectionTitle icon={BarChart3} title="Status das parcelas" subtitle="Pago Ã— Pendente Ã— Atrasado" />
        ...
        {details && (
          <div className="space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <InfoBox label="NÂº do cliente" value={details.client_number} />
              ...
              <InfoBox label="ComissÃ£o total" value={fmtBRL(details.total_commission)} />
              <InfoBox label="CrÃ©dito gerado" value={fmtBRL(details.credit_generated)} />
              <InfoBox label="Seguro" value={details.insurance ? 'Sim' : 'NÃ£o'} />
            </div>
            ...
            <div className="space-y-3">
                        ...
                        <div key={it.number} className="px-4 py-3 flex flex-wrap items-center justify-between gap-3">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <select
                              ...
                              title={isBillOverdue
                                ? 'Cliente estÃ¡ com boleto atrasado.'
                                : (isAutoOverdue ? 'EstÃ¡ atrasada automaticamente (vencida e nÃ£o paga).' : 'Alterar status')}
                            >
              ...
              {detailsTab === 'quotas' && (
                <div className="rounded-3xl border border-slate-200/60 dark:border-white/10 overflow-hidden">
                  <div className="px-4 py-3 bg-slate-50 dark:bg-white/5 border-b border-slate-200/60 dark:border-white/10 font-semibold flex items-center justify-between">
                    <div>Cotas ({editingQuotas.length}) â€¢ Total: {fmtBRL(editingQuotas.reduce((a,v)=>a+(Number(v)||0),0))}</div>
...
function InfoBox({ label, value }) {
  return (
    <div className="rounded-3xl border border-slate-200/60 dark:border-white/10 bg-white/70 dark:bg-white/5 p-4">
      ...
      <div className="mt-1 font-semibold text-slate-900 dark:text-white">{value || 'â€”'}</div>
    </div>
  );
}
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            placeholder="Busca livre: nome, produto, valor, data, status, nÃºmero..."
          ...
          <table className="min-w-full text-sm">
            <thead className="bg-slate-50 dark:bg-white/5 text-slate-600 dark:text-slate-300">
              <tr>
                ...
                <th className="text-right px-4 py-3">ComissÃ£o</th>
              </tr>
            </thead>
