...
import { fmtBRL, fmtDate, statusLabel, boletoLabel, todayIso } from '../utils.js';
import {
  Moon,
  Sun,
  LogOut,
...
function normalizeInstallmentForUi(it) {
  ...
  return {
    ...
    status,
    boleto_unpaid: !!it.boleto_unpaid,
...
export default function Dashboard({ user, onLogout }) {
  ...
  const lateClients = useMemo(() => {
    // Lista "Clientes atrasados": parcelas marcadas como "boleto não pago" e ainda não pagas
    const items = [];
    for (const s of (sales || [])) {
      const its = Array.isArray(s.installments) ? s.installments : [];
      for (const itRaw of its) {
        const it = normalizeInstallmentForUi(itRaw);
        if (it.status === 'paid') continue;
        if (!it.boleto_unpaid) continue;
        items.push({
          sale_id: s.id,
    ...
    // ordena: mais urgente primeiro (vencidas primeiro, depois por vencimento)
    items.sort((a, b) => {
  ...
  const openDetails = (saleId) => {
    const s = (sales || []).find(x => Number(x.id) === Number(saleId));
    if (s) setDetails(s);
  };
  ...
  const setInstallmentUiStatus = (number, newUiStatus) => {
    setEditingInstallments(prev => prev.map(it => {
      ...
      if (newUiStatus === 'paid') {
        return { ...it, status: 'paid', paid_date: todayIso(), boleto_unpaid: false };
      }
  ...
  const setInstallmentBoletoUnpaid = (number, flag) => {
    setEditingInstallments(prev => prev.map(it => {
      if (it.number !== number) return it;
      // se marcar como pago, não faz sentido boleto não pago
      if (it.status === 'paid') return { ...it, boleto_unpaid: false };
      return { ...it, boleto_unpaid: !!flag };
    }));
  };
    ...
    try {
      ...
      const payloadInstallments = editingInstallments.map(it => {
        ...
        return {
          ...
          boleto_unpaid: paid ? 0 : (it.boleto_unpaid ? 1 : 0),
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="pb-4">
            <div className="flex flex-wrap gap-2">
              ...
              <TabButton active={tab==='late'} onClick={()=>setTab('late')} icon={AlertTriangle} label="Clientes atrasados" />
              ...
              <div className="space-y-6">
                <div className="rounded-3xl border border-slate-200/60 dark:border-white/10 bg-white/70 dark:bg-white/5 backdrop-blur-xl p-6 shadow-xl">
                  <SectionTitle
                    ...
                    subtitle="Parcelas marcadas como “boleto não pago” (não é comissão atrasada; é o cliente que não pagou o boleto)"
                  />
                  <div className="mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div className="text-sm text-slate-600 dark:text-slate-300">
                      Total: <span className="font-semibold">{lateClients.length}</span>
                    </div>
                    <div className="text-xs text-slate-500 dark:text-slate-400">
                      Dica: clique em uma linha para abrir a venda e ajustar as parcelas.
                    </div>
                  </div>
                </div>

                <div className="rounded-3xl border border-slate-200/60 dark:border-white/10 bg-white/70 dark:bg-white/5 backdrop-blur-xl overflow-hidden shadow-xl">
                  ...
                  {lateClients.length === 0 ? (
                    <div className="p-6 text-slate-600 dark:text-slate-300">
                      Nenhum cliente marcado como “boleto não pago”.
                    </div>
                  ) : (
                    <div className="overflow-x-auto">
                      <table className="min-w-full text-sm">
                        <thead className="text-left text-slate-600 dark:text-slate-300 bg-slate-50/70 dark:bg-white/5">
                          <tr>
                        ...
                        <tbody className="divide-y divide-slate-200/60 dark:divide-white/10">
                          {lateClients.map((row, idx) => {
                            const st = statusLabel(row.status);
                            return (
                              <tr
                                key={`${row.sale_id}-${row.installment_number}-${idx}`}
                                className="hover:bg-slate-100/60 dark:hover:bg-white/5 cursor-pointer"
                                onClick={() => openDetails(row.sale_id)}
                              >
                                <td className="px-4 py-3">
                                  <div className="font-semibold text-slate-900 dark:text-white">{row.client_name}</div>
                                ...
                                <td className="px-4 py-3 font-semibold">{fmtBRL(row.value)}</td>
                                <td className="px-4 py-3">
                                  <span className={`text-xs px-3 py-1 rounded-full border ${st.cls}`}>{st.text}</span>

                            {(!isPaid && it.boleto_unpaid) && (() => {
                              const bl = boletoLabel(true);
                              return <span className={`text-xs px-3 py-1 rounded-full border ${bl.cls}`}>{bl.text}</span>;
                            })()}
                        ...
                        <div key={it.number} className="px-4 py-3 flex flex-wrap items-center justify-between gap-3">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <label className="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-300 select-none">
                              <input
                                type="checkbox"
                                checked={!isPaid && !!it.boleto_unpaid}
                                onChange={(e) => setInstallmentBoletoUnpaid(it.number, e.target.checked)}
                                disabled={isPaid}
                              />
                              Boleto não pago
                            </label>
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      {Array.isArray(s.installments) && s.installments.some(it => !normalizeInstallmentForUi(it).paid_date && normalizeInstallmentForUi(it).status !== 'paid' && it.boleto_unpaid) && (
                        (() => {
                          const bl = boletoLabel(true);
                          return <span className={`text-[11px] px-2.5 py-1 rounded-full border ${bl.cls}`}>{bl.text}</span>;
                        })()
                      )}
                    </div>
                  </td>
