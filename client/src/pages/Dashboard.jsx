...
export default function Dashboard({ user, onLogout }) {
  ...
  // recebimentos por mês (consultor)
  const [monthRef, setMonthRef] = useState(() => todayIso().slice(0,7)); // 'YYYY-MM'
  ...
  const requestMarkReceived = (row) => setConfirmReceive(row);

  const confirmMarkReceived = async () => {
    if (!confirmReceive) return;

    const saleId = Number(confirmReceive.saleId);
    const number = Number(confirmReceive.number);
    const sale = (sales || []).find(s => Number(s.id) === saleId);
    if (!sale) { setConfirmReceive(null); return; }

    const today = todayIso();
    const payloadInstallments = (sale.installments || []).map(it => {
      if (Number(it.number) === number) {
        return { ...it, status: 'paid', paid_date: today };
      }
      return it;
    });
          ...
          <>
            {tab === 'overview' && (
              <div className="space-y-8">
                ...
                {!isAdmin && (
                  <div className="mt-6 rounded-3xl border border-slate-200/70 dark:border-white/10 bg-white/70 dark:bg-slate-900/50 backdrop-blur p-5">
                    <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
                      <div>
                        <div className="text-sm font-semibold text-slate-700 dark:text-slate-200">Recebimentos do mês</div>
                        <div className="text-xs text-slate-500 dark:text-slate-400">
                          Soma todas as parcelas com vencimento no mês selecionado (e que ainda não estão pagas).
                        </div>
                      </div>

                      <div className="flex items-end gap-3">
                        <div>
                          <div className="text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Mês</div>
                          <input
                            type="month"
                            value={monthRef}
                            onChange={(e) => setMonthRef(e.target.value)}
                            className="h-10 px-3 rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-950/40"
                          />
                        </div>

                        <div className="px-4 py-2 rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-600 text-white shadow-lg shadow-emerald-600/20">
                          <div className="text-xs/4 opacity-90">A receber</div>
                          <div className="text-lg font-bold">{fmtBRL(monthReceivable.total)}</div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mt-4 overflow-x-auto">
                      {monthReceivable.items.length === 0 ? (
                        <div className="text-sm text-slate-500 dark:text-slate-400 py-6 text-center">
                          Nenhuma parcela a receber nesse mês.
                        </div>
                      ) : (
                        <table className="w-full text-sm">
                          ...
                          <tbody>
                            {monthReceivable.items.map((row) => (
                              <tr key={`${row.saleId}-${row.number}`} className="border-t border-slate-200/70 dark:border-white/10">
                                <td className="py-2 pr-3 whitespace-nowrap">{fmtDate(row.due_date)}</td>
                                <td className="py-2 pr-3">{row.client_name}</td>
                                <td className="py-2 pr-3 whitespace-nowrap">{row.product}</td>
                                <td className="py-2 pr-3 whitespace-nowrap">{row.number}ª</td>
                                <td className="py-2 pr-3 whitespace-nowrap">{fmtBRL(row.value)}</td>
                                <td className="py-2 pr-3 whitespace-nowrap text-right">
                                  <button
                                    className="px-3 py-1.5 rounded-2xl bg-emerald-600 text-white hover:bg-emerald-700"
                ...
                {confirmReceive && (
                  <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
                    <div className="w-full max-w-md rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 p-6 shadow-2xl">
                      <div className="text-lg font-bold text-slate-900 dark:text-slate-50">Tem certeza?</div>
                      <div className="mt-2 text-sm text-slate-600 dark:text-slate-300">
                        Marcar como <b>recebido</b> a parcela <b>{confirmReceive.number}ª</b> (venc. <b>{fmtDate(confirmReceive.due_date)}</b>)?
                        <div className="mt-1 opacity-90">
                          <span className="font-semibold">{confirmReceive.client_name}</span> • {confirmReceive.product} • {fmtBRL(confirmReceive.value)}
                        </div>
                      </div>

                      <div className="mt-5 flex gap-3 justify-end">
                        <button
                          className="px-4 py-2 rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-950/30"
