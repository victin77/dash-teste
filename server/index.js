...
function requireDatabaseUrl() {
  // Railway / Render / local may expose different variable names.
  // Prefer DATABASE_URL, but accept common alternatives and strip accidental quotes.
  const candidates = [
  ...
  if (!cs) {
    ...
    if (present) console.error('Variáveis relacionadas encontradas:', present);
    process.exit(1);
  }

  // Remove surrounding quotes if user pasted with quotes in the Variables UI
  cs = cs.trim();
  if ((cs.startsWith('"') && cs.endsWith('"')) || (cs.startsWith("'") && cs.endsWith("'"))) {
    cs = cs.slice(1, -1).trim();
  }
  return cs;
}
...
async function initDb() {
  ...
  const ssl = connectionString.includes('.railway.internal') ? false : { rejectUnauthorized: false };
  pool = new Pool({ connectionString, ssl });

  // Adapter pra manter o resto do código igual (db.get/db.all/db.run/db.exec)
  db = {
    ...
    async run(sql, params) {
      let q = sql;
      const isInsert = /^\s*INSERT\s+INTO\s+\w+/i.test(q);
      const hasReturning = /\bRETURNING\b/i.test(q);
      if (isInsert && !hasReturning) q = q.trim().replace(/;?\s*$/, '') + ' RETURNING id';
      const { text, values } = toPg(q, params);
      const r = await pool.query(text, values);
      return {
    ...
    async exec(sql) {
      // permite múltiplos statements separados por ';'
