...
async function initDb() {
  ...
  await db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS installments (
      ...
      boleto_unpaid INTEGER NOT NULL DEFAULT 0,
      paid_date TEXT,
...
async function upsertInstallments(saleId, totalCommission, saleDate, providedInstallments) {
  ...
  if (!Array.isArray(installments) || installments.length === 0) {
    ...
    installments = installments.map((it, idx) => ({
      ...
      boleto_unpaid: Number(it.boleto_unpaid || 0) ? 1 : 0,
  ...
  for (const it of installments) {
    await db.run(
      'INSERT INTO installments(sale_id, number, value, due_date, status, boleto_unpaid, paid_date) VALUES(?,?,?,?,?,?,?)',
      [saleId, it.number, it.value, it.due_date, it.status, it.boleto_unpaid ? 1 : 0, it.paid_date]
    );
  }
}
...
app.post('/api/sales', auth(), async (req, res) => {
  ...
  const its = await db.all('SELECT number,value,due_date,status,boleto_unpaid,paid_date FROM installments WHERE sale_id=? ORDER BY number', [r.lastID]);
...
app.put('/api/sales/:id', auth(), async (req, res) => {
  ...
  const its = await db.all('SELECT number,value,due_date,status,boleto_unpaid,paid_date FROM installments WHERE sale_id=? ORDER BY number', [saleId]);
...
app.put('/api/sales/:id/quotas', auth(), async (req, res) => {
  ...
  const currentIts = await db.all('SELECT number,value,due_date,status,boleto_unpaid,paid_date FROM installments WHERE sale_id=? ORDER BY number', [saleId]);
