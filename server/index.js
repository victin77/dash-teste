...
// ✅ Troque a senha padrão do admin (pode sobrescrever via ENV ADMIN_PASSWORD)
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || 'Admin26#';

// ✅ Senhas individuais por consultor (pode sobrescrever via ENV CONSULTANT_PASSWORDS_JSON)
const CONSULTANT_PASSWORDS = (() => {
  try {
    const raw = process.env.CONSULTANT_PASSWORDS_JSON;
...
// NOTE: In production (Railway), always set DATABASE_URL.
// If you want to run locally, you can export DATABASE_URL in your terminal.
if (!DATABASE_URL) {
  console.error('❌ DATABASE_URL não definida. Configure no Railway (Variables) ou no seu ambiente local.');
  // Fail fast (prevents the app from starting with an unusable DB)
  process.exit(1);
}

let pool;

/** Convert SQLite-style '?' placeholders to PostgreSQL '$1..$n' */
...
/**
 * Minimal DB wrapper to keep the rest of the code almost unchanged.
 * - db.get(): returns first row or null
 * - db.all(): returns all rows
 * - db.run(): executes and returns { lastID, changes }
 */
const db = {
  ...
  async run(sql, params = []) {
    let sql2 = sql.trim();
    // If it's an INSERT without RETURNING, append RETURNING id so we can emulate sqlite's lastID
    const isInsert = /^INSERT\s+INTO\s+/i.test(sql2);
    if (isInsert && !/\bRETURNING\b/i.test(sql2)) {
      sql2 = sql2.replace(/;\s*$/,'') + ' RETURNING id';
    }
    const q = toPgQuery(sql2, params);
    const r = await pool.query(q.text, q.values);
...
async function initDb() {
  ...
  // Ensure trigger to auto-update updated_at on sales UPDATE (optional)
  await pool.query(`
    DO $$
    BEGIN
      IF NOT EXISTS (
        SELECT 1 FROM pg_trigger WHERE tgname = 'trg_sales_updated'
      ) THEN
  ...
  // Ensure admin user + default consultants/users
  // Ensure admin user
  const admin = await db.get('SELECT id FROM users WHERE username=?', [ADMIN_USER]);
  const adminHash = await bcrypt.hash(ADMIN_PASSWORD, 10);
  if (!admin) {
    await db.run('INSERT INTO users(username,password_hash,role) VALUES(?,?,?)', [ADMIN_USER, adminHash, 'admin']);
    console.log(`✅ Admin criado: ${ADMIN_USER}`);
  } else {
    // garante senha do admin conforme configurada (evita ficar "admin")
    await db.run('UPDATE users SET password_hash=?, role=? WHERE id=?', [adminHash, 'admin', admin.id]);
  }
// Seed default consultants (idempotent)
  ...
  for (const c of consultants) {
    ...
    // define senha por consultor (username é o slug do nome)
    const plain = String(CONSULTANT_PASSWORDS?.[username] || 'Cons24#A');
    ...
    if (!u) {
      ...
      // garante que o consultor continue conseguindo logar com a senha configurada
      await db.run(
        'UPDATE users SET password_hash=?, role=?, consultant_id=? WHERE id=?',
        [hash, 'consultant', c.id, u.id]
      );
    }}
